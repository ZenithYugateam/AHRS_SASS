...
const CompanyDescription: React.FC<CompanyDescriptionProps> = ({ formData, handleInputChange, isEditing }) => {
  const maxLength = 1500;
  const currentLength = formData.companyProfile.description.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${cardStyles} p-4`}>
        <label className="block text-sm font-medium mb-2 text-gray-200">Company Description (200-300 words) *</label>
        {isEditing ? (
          <>
            <textarea
          ...
          <div className={previewTextStyles}>
            {formData.companyProfile.description || 'No description provided'}
          </div>
        )}
      </div>
    </div>
